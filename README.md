# 👋 Welcome to My World

## Hi, I’m **Shimanto**  
🚀 **Full Stack Web Developer (MERN)**  
💻 **Professional Coder | Creative Developer | Problem Solver**

> _“I use animation as a third dimension to simplify experiences and guide users through every interaction — not just for flair, but for meaningful impact.”_

---

## 🧠 About Me

I’m a passionate **Full Stack Developer** who loves turning ideas into real-world digital experiences.  
With a focus on **modern design, clean code,** and **interactive UI/UX**, I build high-performance web applications using the **MERN Stack** and modern tools.

---

## 🧰 Tech Stack I Use

| Category | Technologies |
|-----------|--------------|
| **Frontend** | ![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB) ![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white) ![TailwindCSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat&logo=tailwind-css&logoColor=white) |
| **Backend** | ![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white) ![Express.js](https://img.shields.io/badge/Express.js-000000?style=flat&logo=express&logoColor=white) |
| **Database** | ![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=flat&logo=mongodb&logoColor=white) |
| **Version Control** | ![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white) ![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white) |
| **API & Others** | ![REST API](https://img.shields.io/badge/REST-02569B?style=flat&logo=rest&logoColor=white) |

---

## 🚀 Featured Projects

| Project | Description | Tech Used |
|----------|--------------|-----------|
| 🌍 **[Ali Air Travels & Tours](https://www.aliairtravels.com/)** | A full-featured air travel booking website. | Next.js • Node.js • MongoDB |
| 💬 **[ChatApp](https://shimantochat.vercel.app)** | Real-time messaging app with Firebase. | React.js • Firebase |
| 🧴 **[Joud Perfume Shop](https://www.joudperfume.com)** | E-commerce perfume store. | Next.js • Node.js • MongoDB |
| 📝 **[ToDo List](https://shimanto-todo.vercel.app)** | Real-time ToDo app with Firebase sync. | React.js • Firebase |
| ☁️ **[Weather App](https://react-weather-app-shimantos-projects.vercel.app/)** | Responsive weather forecast app. | React.js • API Integration |

---

## 🖼️ Portfolio

🎯 **Visit my full portfolio:**  
👉 [https://iamshimanto.vercel.app](https://iamshimanto.vercel.app)

💬 I’d love your feedback — let’s connect and collaborate!

---

## 📞 Contact Me

**👤 Name:** Shimanto  
**📱 Phone:** +8801750658101  
**📧 Email:** [shimanto.dev.bd@gmail.com](mailto:shimanto.dev.bd@gmail.com)  

### 🌐 Connect With Me  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/iam-shimanto)  
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/iamShimanto)  
[![Facebook](https://img.shields.io/badge/Facebook-1877F2?style=flat&logo=facebook&logoColor=white)](https://www.facebook.com/iamshimanto18)  
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=flat&logo=firefox&logoColor=white)](https://iamshimanto.vercel.app)

---

## 💬 Feedback Form (Coming Soon)

> “Your thoughts matter! I’m adding a contact form soon so you can directly send messages from my portfolio.”

---

⭐ **If you like my work, don’t forget to star this repo!**  
Thanks for visiting 💖
